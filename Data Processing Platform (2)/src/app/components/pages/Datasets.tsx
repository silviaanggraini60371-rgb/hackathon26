...
import { Search, Filter, Calendar, MapPin, Database, ExternalLink, Download, Sparkles } from "lucide-react";
...
export function Datasets() {
  ...
  return (
    <div className="space-y-6">
      {/* Hero Section */}
      <div className="relative overflow-hidden bg-gradient-to-r from-pink-400 via-purple-400 to-blue-400 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
            <div className="flex-1 text-white">
              ...
              <p className="text-white/80">
                Data BPS Indonesia untuk pendidikan, kesehatan, dan ekonomi 2015-2025
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="text-sm text-gray-700 font-medium">
        Menampilkan <span className="font-bold text-[#f458d1]">{filteredDatasets.length}</span> dataset
      </div>

      {/* Dataset Cards */}
      <div className="grid grid-cols-1 gap-4">
        {filteredDatasets.map((dataset) => (
          <Card key={dataset.id} className="hover:shadow-xl transition-all duration-300 border-2 border-transparent hover:border-purple-200">
            <CardHeader>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <Badge className="text-[rgb(255,255,255)]">{dataset.category}</Badge>
